...
function handleTextInsertionT(text) {
  handleTextInsertion(text, '#tasksTasks-area input[type="text"]');
}

function addTasksTask() {
  var newInput = createInputElement("text", handleTextInsertionT);
  var deleteButton = createDeleteButton();
  var tasksTaskArea = document.getElementById("tasksTasks-area");
  var newTaskDiv = createTaskDiv(newInput, deleteButton, handleTextInsertionT);
  tasksTaskArea.insertBefore(newTaskDiv, tasksTaskArea.lastElementChild);
}
...
function countTasks(taskArea) {
  ...
  document.getElementById("tasksNumber").textContent = tasks.length;
}
...
function initializeTaskAreaObservers() {
  const taskArea = document.getElementById("tasksTasks-area");
  countTasks(taskArea);
...
document.addEventListener("keyup", function (event) {
  if (
    (event.key === " " || event.key === "Enter" || event.key === "Backspace") &&
    event.target.id === "input-box"
  ) {
    let taskDiv = document.querySelector("div.task-name.selected");
    if (taskDiv) {
      let taskId = taskDiv.getAttribute("data-task-id");
...
function saveData(taskId) {
  let mainAreaT = document.getElementById("main-area").innerHTML;
  let inputBoxT = document.getElementById("input-box").innerText;
  localStorage.setItem(taskId, JSON.stringify({ mainAreaT, inputBoxT }));
}
...
function loadData(taskId) {
  ...
  if (savedData) {
    ...
    savedDataHTML = savedData.mainAreaT;
    savedDataTEXT = savedData.inputBoxT;

    if (savedDataHTML) {
      document.getElementById("main-area").innerHTML = savedDataHTML;
      let parantDiv = document.getElementById("notes-area");
      let childDivs = parantDiv.querySelectorAll("div:not([id]");
      childDivs.forEach((div) => div.remove());
      document.getElementById("input-box").innerText = savedDataTEXT;
    }
  }
}
...
function saveMainTaskData() {
  let taskAreaT = document.getElementById("task-area");
  localStorage.setItem("tasksAreaContent", taskAreaT.innerHTML);
}

function loadMainTaskData() {
  let taskAreaT = document.getElementById("task-area");
  taskAreaT.innerHTML = localStorage.getItem("tasksAreaContent");
}

// ----------------------------------------- Load important content on page load -----------------------------------------

window.addEventListener("DOMContentLoaded", () => {
  const mainTaskDataExists = localStorage.getItem("tasksAreaContent");
